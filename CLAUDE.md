...
- **`ui/`** — Stateless rendering. Two-pane layout: left pane (30 chars, repo list with scrolling viewport) + divider + right pane (mode-aware: mode 1 shows branch details, mode 2 shows stash list with selection highlight). Full-screen diff overlay replaces two-pane when active. Context-aware status bar shows 2 modes with mode-specific keybindings. Branch status indicators: green `✔` (clean/even), yellow `●` (ahead/behind with `+N/-N` counts), red `●` (dirty with `N files +X/-Y`), purple `●` (no upstream). Indicators stack. Worktree branches show `[<path>]` annotation, or `[<path1>, <path2>, ...]` when a branch is checked out in more than one worktree. Detached worktrees appear as `(detached)` rows with their path annotation. Up to 5 unpushed commits shown with overflow.
